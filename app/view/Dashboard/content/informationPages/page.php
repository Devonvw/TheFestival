    ...
    fetch(`${window.location.origin}/api/information-page/page?id=${params.get("id")}`, {
        ...
        if (res.ok) {
            ...
            data?.sections?.forEach((section, index) => sectionsHTML +=
                ...
                <label for="${section?.id}" class="block text-sm font-medium text-gray-900">
                    Section ${index + 1}
                </label>
                <button type="button" onclick="deleteInformationSection(${section?.id})"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="text-red-500 w-6 h-6">
...
function deleteInformationSection(id) {
    fetch(`${window.location.origin}/api/information-page/information-section?id=${id}`, {
        headers: {
            'Content-Type': 'application/json'
        },
        method: "DELETE",
