...
class OrderDAO
{
    ...
    public function getOrder($orderId) {
        ...
        $items_stmt = $this->DB::$connection->prepare("SELECT COUNT(oi.id) as quantity, COUNT(oi.id) * oi.price as price, oi.order_id, oi.ticket_id, eis.start, eis.end, eist.price as ticket_price, eist.persons, eist.event_item_slot_id, ei.name as event_item_name, e.name as event_name FROM `order` INNER JOIN order_item as oi on oi.order_id = order.id left join event_item_slot_ticket as eist on oi.ticket_id = eist.id left join event_item_slot as eis on eis.id = eist.event_item_slot_id left join event_item as ei on eis.event_item_id = ei.id left join event as e on ei.event_id = e.id WHERE `order`.id = :order_id GROUP BY oi.order_id, oi.ticket_id, oi.price, eis.start, eis.end, eist.price, eist.persons, eist.event_item_slot_id, ei.name, e.name;");
        ...
        foreach ($items as $row) {
            ...
            array_push($order_items, new OrderItem($row['order_id'], new Ticket($row['ticket_id'], $row['event_item_slot_id'], $row['start'], $row['end'], $row['event_item_name'], $row['event_name'], $row['persons'], $row['ticket_price']), $row['price'],  $row['quantity']));
        }
